...
def mini_batch(batch, mini_batch_size):
    ...
    # print(f"mini_batch old_log_probs={torch.stack(old_log_probs).shape}")
    return torch.stack(states)[rand_ids, :], torch.stack(actions)[rand_ids, :], \
        torch.stack(old_log_probs).reshape(-1, 1)[rand_ids, :], torch.stack(adv)[rand_ids, :], \
...
class PPO2:
    ...
    def __init__(self,
        ...
        if self.anneal_lr:
            self.num_iters = PPO_kwargs['num_episode'] * PPO_kwargs['off_buffer_size'] # batch_size
    ...
    def data_prepare(self, samples: deque):
        ...
        # print(f"state={state.size()}, action={action.size()}, reward={reward.shape}")
    ...
    def update(self, samples_buffer: deque, wandb = None):
        self.update_cnt += 1
        state, action, old_log_probs, advantage, td_target = self.data_prepare(samples_buffer)
