...
|[ CarRacing-v2 ](state: (96, 96, 3),action: (3,)(连续 <-1.0 -> 1.0>))| TD3 | 上述对应配置| ![TD3-car](../pic/SAC_Pusher-v4.gif) |
...
详细看[github test_TD3.py 脚本: Pusher-v4](https://github.com/scchy/RL/blob/main/src/test/test_TD3.py)

tricks:
1. env dealing: 
   1. skip 5 frames
   2. GayScale
   3. judge_out_of_route 
   4. FrameStack
2. CNN
   1. Vanishing gradient problem
   2. MaxPool2d + AvgPool2d
   3. actor: LayerNorm
   4. criticor: Linear Transformer of action before concat
3. train_off_policy
   1. add test during the traing
   2. Saving the best parameters depends on the average reward over multiple tests
